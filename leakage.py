...
class FallTemplateBot2025(ForecastBot):
    ...
    async def run_research(self, question: MetaculusQuestion) -> str:
        """
        a slightly modifiedresearch function to test for temporal leakage, this function makes two retrievals with Perplexity, 
        first is instructed to obey a knowledge cutoff and the second is unrestricted / default retrieval
        """

        async with self._concurrency_limiter:
            researcher = self.get_llm("researcher")
            research_cutoff, research_unrestricted = "", ""
            ...
            base_prompt = clean_indents(
                ...
                Generate a concise but detailed rundown of the most relevant news, including
                if the question would resolve Yes or No based on information up to the cutoff date.
            ...
            # WITH CUTOFF
            cutoff_prompt = f"""
            The current date is {CUTOFF_DATE}.
            Only use information that was available on or before this date.
            If you mention a source, include its claimed publication date in parentheses.
            {base_prompt}
            """

            # WITHOUT CUTOFF
            unrestricted_prompt = f"""
            You may use all currently available information (no cutoff).
            If you mention a source, include its claimed publication date in parentheses.
            {base_prompt}
            """

            if isinstance(researcher, GeneralLlm):
                research_cutoff = await researcher.invoke(cutoff_prompt)
                research_unrestricted = await researcher.invoke(unrestricted_prompt)
            else:
                research_cutoff = await self.get_llm("researcher", "llm").invoke(cutoff_prompt)
                research_unrestricted = await self.get_llm("researcher", "llm").invoke(unrestricted_prompt)

            combined_research = (
                f"WITH_CUTOFF ({CUTOFF_DATE}):\n{research_cutoff}\n\n"
                f"WITHOUT_CUTOFF:\n{research_unrestricted}\n"
            )
            with open("perplexityLeakage.csv", "a", encoding="utf-8") as f:
                f.write(
                    f"{datetime.now().isoformat()},\"{question.question_text}\","
                    f"\"{research_cutoff.replace('\"','')}\","
                    f"\"{research_unrestricted.replace('\"','')}\"\n"
                )
